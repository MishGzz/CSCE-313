	...
	*/
	void* eventThreadHandler(void* a){
		...
		while(1){
			...
			for(int i = 0; i < numWorkers; i++){
				if(channs[i] -> read_fd() > maximum){
					maximum = channs[i] -> read_fd();
				}
				FD_SET(channs[i]->read_fd(), &rfds);
			}
			...
			if(retval){
				for(int i = 0; i < numWorkers; i++){
					if(FD_ISSET(channs[i]->read_fd(), &rfds)){ // channs[i] is ready to read
						string rep = channs[i]->cread();
						reads += 1;
			...
			//else{
				//cout << "No data within tv." << endl;
			//}
			//channel->cwrite(r.str)
			//select()
			//string rep = channel->cread();
			/*
			if(r.requestedID == 0){
				//cout << "rep = " << atoi(r.str.c_str()) << endl;
				joesBuff -> addResponse(r);
...
int main(int argc, char * argv[]) {
	...
	//RequestChannel* requestChannels[numWorkers];
